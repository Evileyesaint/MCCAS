...
### beat1.4采用judge_class_info直接获得任务id，所以这里禁用了

def answer_race(tackid):  #抢答   只抢500次，结束后退出
...
#与answer_race同理，但是不会反馈任何提醒，用于动态更新任务状态，一档有任务立马进入抢答模式      
def answer_race_refresh(tackid_refresh):  #抢答   只抢500次，结束后退出
    if tackid_refresh!=None:   #判断是否为空
        try:
...
def main():
    print('========欢迎使用机电云课堂脚本,当前版本为:v1.1-beta-4=========')
    ...
    try:
        ...
        tackid = judge_class_info(classroom_info)
        if tackid==None:     ##用于判断是否有任务，没有进入自动刷新状态
            while True:
